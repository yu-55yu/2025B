...
config.n1_init = 2.58;
config.n2_real_init = 2.55; % 衬底折射率 n2 拟合初值
...
function [result] = process(data, angle, config, output_filepath)
    ...
    [x_optimal, R_squared_fit] = global_fit_all_parameters(x0, lb, ub, waveNum_fit, R_fit, waveLen_fit, angle);
    ...
    fprintf('   正在将拟合结果写入原始Excel文件的第3列...\n');
    try
        theta0_rad = angle * pi / 180;
        R_fit_full = model_R_vectorized(result.thk, result.n1_complex_full, result.n2, waveNum, theta0_rad);
        ...
        fprintf('   成功将拟合结果保存到: %s (第3列)\n', output_filepath);
    catch ME
        fprintf('   !!! 保存Excel文件时出错: %s\n', ME.message);
        fprintf('   !!! 请确保文件 %s 没有被其他程序打开。\n', output_filepath);
    end

    plot_fit_res(waveNum, R, result.n1_complex_full, result.thk, result.n2, angle, config.waveN_fit_min);
    plot_n1_complex(waveLen_full, waveNum, result.n1_complex_full);
end

%% 全局优化函数
function [x_optimal, R_squared] = global_fit_all_parameters(x0, lb, ub, waveNum_fit, R_fit, waveLen_fit, angle)
    theta0_rad = angle * pi / 180;
...
function R = model_R_vectorized(thk, n1_complex, n2_real, waveNum, theta0)
    n0 = 1.0; n1 = n1_complex(:); n2 = n2_real(:); waveNum = waveNum(:);
...
function R = calculate_R_double_beam(r01_s, r12_s, r01_p, r12_p, exp_term)
    ...
    R_s = abs(r_s_total).^2; R_p = abs(r_p_total).^2;
    R = real((R_s + R_p) / 2);
end
...
function plot_n1_complex(waveLen, waveNum, n1_complex_all)
    n_real = real(n1_complex_all);
    k_imag = imag(n1_complex_all);
    color_n = [0, 114, 189] / 255; % 蓝色
    color_k = [217, 83, 25] / 255;  % 红色
    color_anomalous = [217, 83, 25]/255; % 红色标记点

    figure('Name', '拟合得到的外延层复折射率色散曲线', 'Position', [100, 100, 1200, 450]);
    ...
    hold(ax1, 'on');
    yyaxis(ax1, 'left');
    p1_ax1 = plot(ax1, waveNum, n_real, '-', 'Color', color_n, 'LineWidth', 1.5, 'DisplayName', 'n (正常色散)');
    ylabel(ax1, '折射率实部 n');
    ax1.YColor = color_n;
    
    if any(is_anomalous)
        waveNum_mid_anomalous = 10000 ./ waveLen_mid(is_anomalous);
        n_mid_anomalous = n_mid(is_anomalous);
        p2_ax1 = plot(ax1, waveNum_mid_anomalous, n_mid_anomalous, 'o', 'Color', color_anomalous, ...
            'MarkerFaceColor', color_anomalous, 'MarkerSize', 5, 'DisplayName', 'n (反常色散)');
    end

    yyaxis(ax1, 'right');
    p3_ax1 = plot(ax1, waveNum, k_imag, '--', 'Color', color_k, 'LineWidth', 1.5, 'DisplayName', 'k (虚部)');
    ylabel(ax1, '消光系数 k');
    ax1.YColor = color_k;
    set(ax1, 'YScale', 'log');
    ...
    if any(is_anomalous), legend([p1_ax1, p2_ax1, p3_ax1], 'Location', 'best');
    else, legend([p1_ax1, p3_ax1], 'Location', 'best'); end

    ax2 = nexttile;
    hold(ax2, 'on');
    yyaxis(ax2, 'left');
    p1_ax2 = plot(ax2, waveLen, n_real, '-', 'Color', color_n, 'LineWidth', 1.5, 'DisplayName', 'n (正常色散)');
    ylabel(ax2, '折射率实部 n');
    ax2.YColor = color_n;
    
    if any(is_anomalous)
        p2_ax2 = plot(ax2, waveLen_mid(is_anomalous), n_mid(is_anomalous), 'o', 'Color', color_anomalous, ...
            'MarkerFaceColor', color_anomalous, 'MarkerSize', 5, 'DisplayName', 'n (反常色散)');
    end
    
    yyaxis(ax2, 'right');
    p3_ax2 = plot(ax2, waveLen, k_imag, '--', 'Color', color_k, 'LineWidth', 1.5, 'DisplayName', 'k (虚部)');
    ylabel(ax2, '消光系数 k');
    ax2.YColor = color_k;
    set(ax2, 'YScale', 'log');
    ...
    grid(ax2, 'on');
    xlabel(ax2, '波长 (μm)');
    if any(is_anomalous), legend([p1_ax2, p2_ax2, p3_ax2], 'Location', 'best');
    else, legend([p1_ax2, p3_ax2], 'Location', 'best'); end
end


% 绘制拟合结果
function plot_fit_res(waveNum, R, n1_complex_full, thk, n2_real, angle, waveN_fit_min)
    color_blue = [0, 176, 232] / 255;
    color_red = [254, 100, 108] / 255;
    theta0 = angle * pi / 180;
    R_fit = model_R_vectorized(thk, n1_complex_full, n2_real, waveNum, theta0);
