...
function [res] = process(data, angle, config, output_filepath)
   ...
   ...
    ...
    try
        ...
        R_fit_full = compute_R(res.thk, res.n1_complex, res.n2_complex_full, waveNum, theta0_rad, 1);
    ...
    % 柯西
    subplot(1,2,1);
    R_fit_cauchy = compute_R(cauchy_res.thk, cauchy_res.n1_complex, cauchy_res.n2_complex_full, waveNum, angle * pi / 180, 1);
    ...
    % sel
    subplot(1,2,2);
    R_fit_sel = compute_R(sel_res.thk, sel_res.n1_complex, sel_res.n2_complex_full, waveNum, angle * pi / 180, 1);
    plot(waveNum(2:end), R(2:end)*100, '-', 'Color', '#d74f44', 'LineWidth', 1.5, 'DisplayName', '实验数据');
    hold on;
