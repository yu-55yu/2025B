...
function [result] = process(data, angle, config, output_filepath)
    ...
    % --- 柯西模型子图 ---
    subplot(1,2,1);
    R_fit_cauchy = compute_R(cauchy_result.thk, cauchy_result.n1_complex_full, cauchy_result.n2_complex_full, waveNum, angle * pi / 180, 1);
    ...
    % --- Sellmeier模型子图 ---
    subplot(1,2,2);
    R_fit_sellmeier = compute_R(sellmeier_result.thk, sellmeier_result.n1_complex_full, sellmeier_result.n2_complex_full, waveNum, angle * pi / 180, );
    % 使用 #008ede 颜色绘制实验数据
    plot(waveNum(2:end), R(2:end)*100, '-', 'Color', '#d74f44', 'LineWidth', 1.5, 'DisplayName', '实验数据');
    hold on;
