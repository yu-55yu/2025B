...
function [res] = process(data, angle, config, output_filepath)
    ...
    res.n1_complex_full = cal_n_sellmeier(waveLen_full, res.selParam, res.k1Param);

    fprintf('    最终厚度: %.2f μm\n', res.thk);
    fprintf('    最终外延层 n1 (在 6μm): %.3f + %.4fi\n', real(interp1(waveLen_full, res.n1_complex_full, 6)), imag(interp1(waveLen_full, res.n1_complex_full, 6)));
    fprintf('    最终衬底 n2: %.3f\n', res.n2);
    ...
    R_fit_full = compute_R(res.thk, res.n1_complex_full, res.n2, waveNum, theta0_rad,1);
    ...
    plot_optical_constants(waveLen_full, waveNum, res.n1_complex_full, '外延层 (Sellmeier)');
end
